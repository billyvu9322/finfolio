...
export function ReportsPage() {
  ...
  const input =
    "rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm outline-none focus:border-brand";
  ...
  const mapRenderLabel = (type: string) => {
    switch (type) {
      case "gold":
        return "Vàng";
      case "stock":
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-neutral-400">
        P&L theo loại tài sản, AUM theo tháng, export CSV và xem snapshot lịch
        sử.
      </p>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-3">
        ...
        <select
          value={module}
          onChange={(event) =>
            setModule(event.target.value as "gold" | "stock")
          }
          className={input}
        >
        ...
        <button
          onClick={() => exportCsv(module, from || undefined, to || undefined)}
              ...
              {report.data?.byAsset.map((asset) => (
                ...
                >
                  <td className="py-2 font-sans">
                    {mapRenderLabel(asset.assetClass)}
                  </td>
                  <td className="py-2 text-right">{formatVnd(asset.value)}</td>
                  <td className="py-2 text-right">
                    {formatVnd(asset.invested)}
                  </td>
                  <td
                    className={`py-2 text-right ${Number(asset.pnl) >= 0 ? "text-profit" : "text-loss"}`}
                  >
                    {formatVnd(asset.pnl)}
                  </td>
                  <td
                    className={`py-2 text-right ${Number(asset.pnlPct) >= 0 ? "text-profit" : "text-loss"}`}
                  >
                    {asset.pnlPct}%
                  </td>
                </tr>
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={(report.data?.byMonth ?? []).map((item) => ({
                  month: item.month,
                  aum: Number(item.aum),
                }))}
              >
      ...
      <div className="mt-6 rounded-xl border border-neutral-800 bg-neutral-900 p-5">
        <h2 className="mb-3 text-base font-semibold">
          Xem danh mục theo ngày (Snapshot)
        </h2>
        <div className="flex items-end gap-3">
          <input
            type="date"
            value={snapDate}
            onChange={(event) => setSnapDate(event.target.value)}
            className={input}
          />
          <button
        ...
        {snap ? (
          <div className="mt-3 grid grid-cols-1 gap-4 font-mono text-sm sm:grid-cols-3">
            <div>
              <div className="text-neutral-400">Ngày</div>
              {snap.snapshotDate}
            </div>
            <div>
              <div className="text-neutral-400">Giá trị</div>
              {formatVnd(snap.totalValue)}
            </div>
            <div>
              <div className="text-neutral-400">Vốn</div>
              {formatVnd(snap.totalInvested)}
            </div>
          </div>
        ) : null}
      </div>
